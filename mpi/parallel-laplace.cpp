...
/*
  ...
  As a first attempt to parallelise the Laplace’s solver, divide the
  grid horizontally into bands, one for each process. 

  In the first instance, use calls to the blocking point-to-point
  communication functions MPI Send and Recv to obtain the values at the
  boundary of each process’ (ghost cells), from the correct neighbour.
  At the end of the simulation, send the results back to rank 0 in
  order, so that it writes the output file. 
...
int main()
{
  ...
  for (int n = 0; n < niter; n++) {
    ...
    MPI_Send(&g1(end - 1, 0), SIZE, MPI_DOUBLE,
    ...
    MPI_Recv(&g1(-1, 0), SIZE, MPI_DOUBLE,
    ...
    MPI_Send(&g1(0, 0), SIZE, MPI_DOUBLE,
	     (rank == 0 ? MPI_PROC_NULL : rank - 1),
  ...
  if (rank == 0) {
    ...
    MPI_Send(&g1(0, 0), (end - start) * SIZE, MPI_DOUBLE,
